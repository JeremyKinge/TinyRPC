    ...
    public static Object create(final Class clazz){
        ...
        return Proxy.newProxyInstance(clazz.getClassLoader(), new Class[]{clazz}, new InvocationHandler() {
            public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                ...
                classInfo.setClassName(cn);
                classInfo.setMethodName(method.getName());
                classInfo.setArgs(args);
                classInfo.setClazzType(method.getParameterTypes());

                EventLoopGroup eventExecutors=new NioEventLoopGroup();    //创建一个线程组，默认创建的线程时cpu核心数的两倍
                //创建客户端启动助手  完成相关配置
                Bootstrap bootstrap=new Bootstrap();
                //创建业务处理类，这个是入站处理器，服务端返回数据后会经过这个处理器处理
                    ...
                    bootstrap.group(eventExecutors)    //设置线程组
                            ...
                            .handler(new ChannelInitializer<SocketChannel>() {
                                ...
                                protected void initChannel(SocketChannel socketChannel) throws Exception {
                                    ...
                                    //添加编码器，这个是出站处理器，客户端请求数据给服务端时会经过这个处理器
                                    pipeline.addLast("encoder",new ObjectEncoder());

                                    //添加解码器，入站处理器，服务端返回数据时，会经过这个客户端的处理器进行过数据处理
                                    //作用就是反序列化，字节码数据成对象
